...
test('camera fingertip selects all materials, pinch selects, and confirmed actions learn locally',async({page})=>{
  ...
  await expect(page.locator('#learningCount')).toContainText('0 confirmed actions');
  ...
  await page.reload();
  await expect(page.locator('#learningCount')).toContainText(learned.successes+' confirmed actions');
  ...
  await pointAt(page,'#forgetLearning');
  await expect(page.locator('#learningCount')).toContainText('0 confirmed actions');
  await expect(page.locator('#toast')).toContainText('Local learning reset');
...
test('worker failure preserves basic materials and gesture menu',async({page})=>{
  ...
  await page.goto('/');await expect(page.locator('#intentStatus')).toHaveText('Assist unavailable');
